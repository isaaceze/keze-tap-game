...
if __name__ == '__main__':
    ...
    if available_port is None:
        print(f"❌ Could not find an available port starting from {preferred_port}")
    ...
    if available_port != preferred_port:
        ...
        print(f"💡 Update your frontend API URL to: http://localhost:{available_port}/api")

    debug = os.getenv('FLASK_DEBUG', 'False').lower() == 'true'

    print(f"🚀 Keze Tap Game MySQL server starting on port {available_port}")
    print(f"🔗 API URL: http://localhost:{available_port}/api")
    ...
    app.run(host='0.0.0.0', port=available_port, debug=debug)
